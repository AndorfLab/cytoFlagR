...
This R Markdown file **cannot be knitted**.

Several analysis steps in this workflow (for example, selecting folders and files in pop-up windows) require interactive user input, which is not available during knitting. Attempting to knit this document will result in errors.

For this reason, you should run each chunk manually using:
  
  - the 'Run' button in RStudio (top right corner), or  
- 'Ctrl+Shift+Enter' / 'Cmd+Shift+Enter' to run the current chunk.

More details on using R Markdown can be found here: <http://rmarkdown.rstudio.com>.

### CytoBatchFlagR Overview ###

This tool consists of 6 main parts. More information about CytoBatchFlagR can be found here: <https://github.com/AndorfLab/CytoBatchFlagR/wiki>. 

* 1. Pre-processing and Visual Assessment of the Data
--Transforms control samples (FCS files) and creates various plots (a MDS, UMAP, and bar plot) for initial data analysis 

* 2. Interquartile Range (IQR)-Based Assessment
...
* 3. Earth Mover’s Distance (EMD)-Based Assessment
--Uses the EMD equation for pairwise comparisons between every marker in the control samples

* 4. Summary of Results (for Steps 2 and 3)
--Creates a plot that summarizes the flags from the IQR-based and EMD-based assessments

* 5. Unsupervised Clustering-Based Assessment
--Clusters the data to highlight batch issues present within the cell populations

* 6. Summary of Results (for Steps 2, 3, and 5)
--Creates a plot that summarizes the flags from the IQR-based, EMD-based, and cluster-based assessments

### Example data ###

The current functions and parameters in this file are set to run with the example spectral data that is on GitHub. If using your own data, please remember to change parameters as necessary. 

### Installing and Loading Dependencies ###

Before running this tool, please ensure that all required R packages are installed and available in your environment.
...
# Packages needed
requiredPackages <- c("dplyr","scales","tidyr","reshape2","readr",
                    "matrixStats","readxl","ggplot2","ggpubr",
                    "ggridges","MASS","RColorBrewer","cowplot",
                    "ggrepel","emdist","circlize","gridExtra","stats",
                    "LaplacesDemon","pheatmap","umap","progress","crayon",
                    "patchwork","ggpmisc","viridis","tidyverse","shiny",
                    "shinyjs","DT","bslib","shinyjs","cluster","rstudioapi",
                    "grid","shinycssloaders","shinyWidgets")
# Install
...
If you haven't done so already, install Bioconductor Manager.

```{r Optional: install BioConductor package manager}

# From Bioconductor, install Bioconductor Manager (BiocManager) and then install Bioconductor packages ("https://www.bioconductor.org/install/")

checkBiocManager_install()

```

Next, install the Bioconductor packages.
...
Once packages are installed, load the libraries into R.
...
Save the following R files that contain the CytoBatchFlagR functions in the **same folder** where this RMD file exists.              

Using *source()* allows you to read in the R functions that make up CytoBatchFlagR.

```{r CytoBatchFlagR functions: make sure you use the file path to where these files are stored}

# You may have to set the path that contains this Rmd file and the R files below. Example: 
# setwd("C:/your/path/here/CytoBatchFlagR")
...
```{r Load fcs metadata file, marker information file}
...
```{r Set up transformation type and transform data in FCS files, echo=FALSE}
...
Next, density plots and biaxial plots can be created to visualize the data and help determine if the automated marker thresholds are sufficient or if they should be overrided. 
...
If your data has a large amount of batches, you may want to split the data into multiple plots to improve readability. The batches_per_plot parameter can be used in this case. batches_per_plot indicates the maximum number of batches to include in each plot. For example, if you have 12 batches and batches_per_plot = 5, then for each marker you will have 3 plots (plot 1: batches 1-5, plot 2: batches 6-10, plot 3: batches 11-12). batches_per_plot = NULL is used if you want to include all batches.
...
```{r biaxial dot plots of marker densities against a reference marker}
...
Using the thresholds, get the negative population, positive population, and percent positive cells for all markers and controls.
...
Now, you can generate a dataframe that contains flags for all 3 IQR metrics above (-MFI, +MFI, and %pos).
...
Default threshold is 5, but this can be changed based on your data. 
...
Finally, create a dataframe of median EMD values and the corresponding generated flags. 
...
```{r Ranked, summarized heatmap of IQR-based and EMD-based assessment metrics}
...
- Selecting markers in a pop-up window, or
- If a 'status' column was included in the marker file, then filter by values in that column
...
```{r Select parameters for clustering assessment}
...
```{r Generate bar plot of cluster sizes}
...
```{r Bar plot of batch proportions per cluster; list of batches > 2x expected proportion saved}
...
In the plot, flags that signify a batch above the threshold are marked with the name of that batch next to the flagged point. In contrast, a point without a label indicates it was not flagged. Only clusters that have at least one batch above the threshold are included in the outputted plot. 
...
Finally, a UMAP that colors by batch is made as a final visualization of the clustering assessment.
...
Once Steps 2, 3, and 5 are completed, all IQR-based, EMD-based, and cluster-based flags have been generated. As a result, an overall summary of flags from all metrics can be created.
...
The outputted metrics and figures should provide you with a comprehensive assessment of your dataset. Now, you can determine the right solution to correct for these potential batch effects. It may be best to exclude potential problematic batches or markers altogether from your downstream analysis. Alternatively, you can apply a suitable batch normalization algorithm to your data.
